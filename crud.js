...
const obtenerUsuarios = async () => {
    try {
        ...
        console.log(error);
        throw new Error("Error al traer los datos de los usuarios.");
    }
};
...
const transferencia = async (idEmisor, idReceptor, monto) => {
    try {
        // Comprobar que el usuario emisor tiene saldo superior al monto a transferir
        ...
        // Comprobar que ambos usuarios existen en la tabla
        ...
        if (!emisorExists.rows[0].exists || !receptorExists.rows[0].exists) {
            return res.status(400).json({ message: 'No se pudo encontrar usuario emisor o receptor en los registros.' })
        }
        await pool.query('BEGIN')
        // Actualizar balance Emisor y receptor
...
const registroTransferencias = async (idEmisor, idReceptor, monto) => {
    try {
        ...
        console.log('Error en la consulta a la base de datos', error);
        throw new Error("Error al agregar registro nueva transferencia.");
    }
};
...
const obtenerTransferencias = async () => {
    try {
        ...
        console.log(error);
        throw new Error("Error al traer los datos de las transferencias.");
    }
};
